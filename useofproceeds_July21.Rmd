...
Over 1800 observations do not have principle values. After randomly checking links and looking for patterns within missing data, these observations will ultimately be dropped from the analysis. See bonddata_june23.rmd for data cleaning details. 
...
cities_keep <- all_data %>% 
  ...
  fed_taxed = case_when(
      ...
      str_detect(security_description, "FEDERALLY.*TAX|TAXABLE|\\b\\s\\sAMT.*\\b") ~ "Taxable",
      TRUE ~ "Unknown")) %>%
...
- Used as guide but not did not keept all categories  
...
`cities_keep` includes only new bond observations (i.e. not refinancing, refunding, remarketing, etc.) that had principle values in the data and mature before 2062.
...
- Grouping by Issuer causes some problems due to inconsistencies with names. Could use cusip_identifier but not always there. Other option is use the first 7 digits from CUSIP since that is the equivalent of the cusip id for issuers.   
...
Other Bonds:
```{r}
bondseries2 %>% filter(rev_GO == "Other Bond")
...
```{r use-of-proceeds-issuedetails, warning=FALSE, error=TRUE}
cap_improve <- c("MULTIMOD|BUILD.*AMER|BRIDGE|BRDG|BELTWAY|BELTLINE|TRANSPO|CAP.*APPREC|CAPITAL.*APP|RAPID.*TRAN")
# includes capital words, Build America, infrastructure, and transportation words
pub_utilities <- c("PUBLIC.*UTIL")
pub_improve <- c("\\bPUB.*IMP|PUBLIC.*IMP|IMPVT|\\bIMPT")
pub_safety <- c("PUBLIC.*SAFETY|PUB.*SAF|SAFETY|FIRE.*DEPT")
LLC <- c("LLC|\\bLLC")
airport <- c("AIRPORT|ARPT")
development <- c("DEVELOPMENT|//bREDEV|DEV.*AUTH|TAX.*ALLO|TAX.*INC|ENTERPR|OPPORTUNITY|TIF|ECONO")
# development includes TIF words. Does NOT include Industrial Development. 
# unclear on difference between development, redevelopment, and indus. dev.

education <- c("UNIVERSITY|CAMPUS|UNIV|COLLEGE|SCHOOL|\\bSCHL\\b|\\bSCH\\b |STUDENT|ACADEMY|CHARTER")
# All education. Schools, charters, academies, universities, campus, etc.
...
parks_beaches <- c("PK|\\bPARK\\b|BEACH|RECREATION")
municipal_facilities <- c("NEIGHBORHOODS.*ALIVE|NEIGHBORHDS.*ALIVE")
settlements <- c("SETTLEMENT.*OB")
...
bondseries2 <- bondseries2 %>%
  mutate(
   ...
   #   str_detect(issue_details, tif_econdev) ~ "TIF/Economic Zones",
      ...
      str_detect(issue_details, settlements) ~ "Settlements", 
...
There are 391 Unknowns. If we want slightly more detail, we can code them as Revenue, GO, or Other bonds. Ex. if  `use_of_proceeds_issuedetails == "Unknown"` and `rev_GO == Revenue Bond`, then it will code it as a generic revenue bond. This is done below where there are now 64 Unknown instead of 391:

```{r use-of-proceeds-issuedetails2, warning=FALSE, error=TRUE}
...
If you want to look at those that were coded as Unknown:
```{r use-of-proceeds-issuedetails-unknown}
bondseries2%>%filter(use_of_proceeds_issuedetails == "Unknown")
```

Main themes for Unknown proceed types include words like "ARS", "Subseries", "Cap Apprec", Direct Access, CPI, FLOATS, etc. These words are mostly captured in the Bond Type variables created later. 


### Security Description 

Many of these (below) capture the use of proceeds, not the bond security method despite the variable name. The `security_description` variable is better for other information like rate type, bond type, funding type, etc. but in this section we check for additional use of proceeds information, just in case its included in this variable but not the issue_details.  

- Can combine security_description use of proceeds with the issue_detail use of proceeds. If both variables determine a use of proceeds. Check if they are they same.  
...
> This is not as thorough as the issue_details_useofproceeds. Mostly done to as code example for pulling use of proceeds from this variable. Potentially could be combined with issue_details use of proceed information if desired. I did not do this. 


```{r use-proceeds-securitydesc}
...
# types of centers: transit, community, youth, health, convention, center for ...., ctr strategic & intl studies, research center, treatment center ....., 
community_ctr <- c("YOUTH.*CTR|\\bCOMMUNITY.*CTR|RECREATION")
...
To do: Most remaining observations are difficult to identify the use of proceeds. Code other variables first and maybe we can guess the use of proceeds later depending on if they are taxable, go bonds, etc.



## Authorities

This variable would ideally be coding who was doing the borrowing (e.g. Housing authorities, transportation authorities, etc.). Obviously different cities have different authorities that may have overlapping jurisdictions. This was just an attempt to see what kind of authorities could be easily identified in the data to see if it was useful. I currently do not know if it is useful, but it may be some day. 

> Related to "Issuer Details" Section below. 
...
Affordable, Multifamily|Multi-family|Multi-fam, Multi-family assisted living, Apartments, Housing opportunity program?, Single Family, Housing Authority using Build America Bonds, Collaterized multi-family mortgage bonds, Senior apartments

> Technical detail for case_when(): if the values have been replaced once, they won’t be replaced again when using case_when.
...
mutate(
  housing = case_when(
    str_detect(security_description, c("SENIOR.*LIEN"))~"NonHousing", # kept getting pulled into senior housing so coding it specifically as NonHousing
...
```{r housing}
bondseries2 %>% filter(housing != "NonHousing") # NOT equal to NonHousing
#329 have some type of housing word
```

Observations that were coded as Housing Authority from the `authorites` variable:
```{r}
bondseries2 %>%filter(authorities == "Housing Authority")
```

Cross table of authorities and housing types:
```{r}
table(bondseries2$authorities, bondseries2$housing)

```

## Bond Type, Rate, Other Variables

Two main types: GO bonds and revenue bonds. Coded at beginning of document in `rev_GO`. Besides just General Obligation Bonds and Revenue Bonds, additional detail on the bonds were included for many observations:
...
Facility Bonds has 71 occurrences in code chunk below and 64 occurrences above so there must be some overlap in categories.  
- To do: Use list of tax exempt facility bonds to expand on this if it might be useful.  
...
# Merged Data - Descriptive Stats!

After creating the variables for the grouped bond series above, we can merge them back to larger data set of individual bonds at the CUSIP level of observation. This will take all the categorical info created and apply it to each individual bond within a bond series. 
...
joined_filtered <- joined %>%  # doublecheck that all of our filtering still applies
  filter(principle_amount_at_issuance > 0,
         ref_string == "Keep",
         maturity_date <= '2062-01-01')  %>%
      mutate(issue_year = year(issue_date))
# joined == joined_filtered. same number of observations, this is good.
```

Made a list of unique issue_details. This shows first 6 as an example. `obs` is number of times the issue_detail shows up in the data.  There are 1290 unique issue_details found. 

```{r}
issuedetails_list <- cities_keep %>% 
  group_by(issue_details) %>% 
  summarize(obs = n()) %>%
  select(obs,issue_details)

head(issuedetails_list, 6)  #1290 obs
```

If we wanted to look at just the issuers, we can group by `issuer` and count the number of times each issuer appears in the dataset (`obs`). There are 94 issuers within the 5 cities (but many of them are very similar in name and probably are the same entity). 
...
After data cleaning, 9283 observations are used in the graphs and tables below. 
```{r graph}
joined_filtered %>% 
  group_by(city) %>% 
  ggplot() + 
...
Year for the cutoff in the graphs and descriptive stats is 2052. DC had 1 bond series that goes to 2062, but we want a similar comparison between the cities.
